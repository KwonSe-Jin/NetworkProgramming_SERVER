#include "Animals.h"

Animals::Animals() : Unit(glm::mat4(1.f)), Scale(glm::mat4(1.f)), Trans(glm::mat4(1.f)),
Rotate(glm::mat4(1.f)), Change(glm::mat4(1.f)), Color(glm::vec3(1.f)), Position(glm::vec3(0.f, -1.f, 0.f))
{
}

Animals::~Animals()
{
}

void Animals::draw()
{
	BindVAO();
	GLuint SelectColor = glGetUniformLocation(shaderID, "SelectColor");
	glUniform1i(SelectColor, 1);
	GLuint aColor = glGetUniformLocation(shaderID, "objectColor");
	glUniform4f(aColor, this->Color.r, this->Color.g, this->Color.b,1.);
	GLuint modelLocation = glGetUniformLocation(shaderID, "modelTransform");
	glUniformMatrix4fv(modelLocation, 1, GL_FALSE, glm::value_ptr(Change));
	DrawArrays();

}
void Animals::keyIn(glm::vec3 Position, float Direction)
{
	this->Position = Position;
	this->Direction = Direction;

	update();

}