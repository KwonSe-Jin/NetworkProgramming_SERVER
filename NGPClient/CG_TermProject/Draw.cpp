...
void get_vangleandStatus(float* x, float* y, bool* status, float* dirx, float* diry, float* dirz, string * name,bool* restart)
{
    hero[global_ID].get_Info(x, y, status, dirx, diry, dirz, name,restart);
}
...
void draw() {
    ...
    //for (int i = 0; i < 3; ++i) {
        ...
        //++cnt;
    //}
    if (RestartPress && hero[global_ID].Restart == true) {
        Restartinit();
        RestartPress = false;
    }
